...
# --browser flag controls which browser (chromium/firefox/webkit)
# --headed flag runs visibly; default is headless for CI compatibility
...
# Shared by both UI tests (login/accounts/transfer) and API tests (REST auth)
# Uses a uuid suffix on the username to guarantee uniqueness on every run
@pytest.fixture(scope="session")
def registered_user(browser_instance, base_url, test_data):
    ...
    logger.info(f"Registering session user: {user_data['username']}")
    registration_page.register(user_data)

    # Confirm registration succeeded before any test is allowed to proceed
    expect(registration_page.get_success_locator()).to_contain_text(
        f"Welcome {user_data['username']}", timeout=5000
    )
    logger.info(f"Registration confirmed for: {user_data['username']}")
    ...
    # Clicking before this loads causes a silent backend failure — form sends no account ID
    ...
    logger.info("Session user setup complete — registration and second account confirmed")
    context.close()
...
# ── Test data fixture ──────────────────────────────────────────────────────────
# Scope: session — loads registration_data.json once and shares across all tests
@pytest.fixture(scope="session")
def test_data():
